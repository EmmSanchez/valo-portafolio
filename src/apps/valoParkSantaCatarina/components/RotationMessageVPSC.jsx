...
import SocialIcon from "@/apps/valoPortafolio/components/Buttons/social-icon";
import Whatsapp from "@/apps/valoPortafolio/assets/icons/social-icons/whatsapp";
import Web from "@/apps/valoPortafolio/assets/icons/social-icons/web";
import Calendar from "@/apps/valoPortafolio/assets/icons/calendar";
import LogoValoParkSantaCatarina from "../assets/logos/logo-valoParkSC";

export default function RotationMessageVPSC() {
  return (
    <div className="flex flex-col w-full h-dvh p-[clamp(18.5px,3.255208vw,25px)] bg-white/55 backdrop-blur-md">
      <div className="flex flex-col shrink-0">
        {/* Botón cerrar */}
        <button className="self-end w-fit h-fit p-[clamp(5.02px,0.885417vw,17px)] hover:cursor-pointer bg-santa-catarina">
          <CerrarIcon className="size-[clamp(13.31px,2.34375vw,45px)] text-white" />
        </button>
        <LogoValoParkSantaCatarina className="self-center w-[clamp(150px,19.53125vw,200px)] h-[clamp(45.75px,5.957031vw,61px)]" />
      </div>
      <div className="flex flex-col grow justify-center items-center gap-[clamp(29.58px,5.208333vw,40px)]">
        ...
        {/* Texto */}
        <p className="font-lumarc uppercase text-center leading-none text-santa-catarina">
      ...
      <div className="flex shrink-0 self-center gap-[clamp(11.83px,2.083333vw,16px)]">
        <SocialIcon
          icon={Whatsapp}
          href={"#"}
          className="text-white group-hover:text-santa-catarina"
          bgColor="bg-santa-catarina"
