        ...
        {filters.map((button) => {
          ...
          return (
            <button
              ...
              className={`group flex-1 flex justify-between items-center px-[clamp(5.92px,1.04vw,20px)] text-white font-semibold rounded-[clamp(11.83px,2.08vw,40px)] hover:cursor-pointer ${activeFilter === button.id ? "bg-santa-catarina-grey" : "hover:bg-santa-catarina-grey"}`}
            >
              ...
              <span className="size-[clamp(7.99px,1.41vw,27px)]">
                {IconComponent ? (
                  ...
                  <>
                    <CircleIcon
                      className={`group-hover:fill-white ${activeFilter === button.id && "fill-white"}`}
                    />
                  </>
                )}
              </span>
