...
export default function NavesVideoTour() {
  ...
  return (
    <div className="flex w-dvw h-dvh default-padding pointer-events-none">
      ...
      <div className="absolute h-full right-[clamp(10.36px,1.82vw,35px)] top-1/2 -translate-y-1/2 flex flex-col justify-center gap-[clamp(2.96px,0.52vw,10px)] pointer-events-none">
        <div className="flex flex-col justify-between lg:w-[clamp(116.87px,20.57vw,395.1px)] lg:h-[clamp(145.19px,45.37vh,490px)] p-[clamp(2.96px,0.52vw,10px)] gap-[clamp(2.96px,0.52vw,10px)] bg-santa-catarina">
          {CARACTERISTICAS.map((caracteristica) => {
            ...
            return (
              <button
                ...
                className={`flex justify-between items-center h-full max-lg:py-1 px-[clamp(7.39px,1.30vw,25px)] max-lg:gap-4 rounded-[clamp(14.8px,2.6vw,50px)] pointer-events-auto ${mode !== MODE.TRANSITIONING ? "hover:cursor-pointer hover:bg-santa-catarina-grey" : ""} ${activeCaracteristica === caracteristica.slug ? "font-bold bg-santa-catarina-grey" : "bg-santa-catarina"}`}
              >
