...
import pos5 from "@/apps/foro4/assets/images/masterplan/rotacion-exteriores/foro4-pos5.webp";
...
"
      ...
      <div className="absolute bottom-0 right-0 w-full flex justify-end default-foro4-padding pointer-events-none">
        <Link
          to={"/foro4/masterplan/proyecto"}
          data-active={position === 5}
          disabled={mode === MODE.TRANSITIONING}
          onClick={() => handleChangePosition(5)}
          className={`group relative flex justify-end items-center w-[clamp(62px,19.231vw,140px)] lg:w-full max-w-[215px] h-[clamp(23.63px,4.166667vw,80px)] px-[clamp(5.02px,0.885417vw,17px)] py-[clamp(2.95px,0.520833vw,10px)] rounded-[clamp(14.8px,2.6vw,50px)] shadow-lg border border-white bg-foro4-morado overflow-hidden ${mode !== MODE.TRANSITIONING ? "hover:cursor-pointer pointer-events-auto" : "pointer-events-none"}`}
        >
          <img
            src={pos5}
            alt={`Posición 5`}
          ...
          <div
            className={`
                absolute inset-0 bg-foro4-morado transition-opacity duration-300
                ${position === 5 ? "opacity-30" : "opacity-80"}
              `}
          />
