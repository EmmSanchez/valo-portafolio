...
export default function MainMenuForo4() {
  const { menu } = data;

  return (
    <div className="w-[clamp(180px,16.2vw,311px)] px-1 py-2 2xl:px-5 2xl:py-6.25 bg-white">
        ...
        {menu.map((section) => {
          return (
            <Link
              key={section.id}
              to={section.to}
              className="group flex items-center justify-between w-full h-[clamp(32px,3.65vw,70px)] px-6.25 text-mobile-button lg:text-paragraph-button font-semibold rounded-[40px] hover:bg-foro4-morado hover:font-bold hover:cursor-pointer"
            >
              <span className="text-foro4-morado group-hover:text-white">
                {section.label}
              </span>
